...
SKILL_SCRIPT = (
    Path(__file__).parent.parent.parent / "docs" / "skill" / "vidgen_agent.py"
)
SKILL_DOCUMENT = SKILL_SCRIPT.with_name("SKILL.md")
SPEC = importlib.util.spec_from_file_location("vidgen_agent_skill", SKILL_SCRIPT)
vidgen_agent = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(vidgen_agent)
...
class TestMptAgentSkill(unittest.TestCase):
    ...
    def test_skill_runs_helper_from_its_working_directory(self):
        ...
        self.assertIn(
            'uv run --no-project --python 3.11 python vidgen_agent.py --subject',
            text,
        )
        self.assertIn("workdir=SKILL_DIR", text)
        self.assertNotIn('python "<SKILL_DIR>/vidgen_agent.py"', text)

    def test_first_run_only_requests_missing_api_keys(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir) / "VidGen"
            self.create_project(root)
            output = io.StringIO()

            with patch.dict(os.environ, {}, clear=True), redirect_stdout(output):
                code = vidgen_agent.main(
                    ["--subject", "人工智能如何改变生活", "--root", str(root)]
                )

            self.assertEqual(code, vidgen_agent.NEEDS_INPUT_EXIT_CODE)
            text = output.getvalue()
            self.assertIn("VIDGEN_NEEDS_INPUT", text)
            ...
            self.assertIn(
                f"PEXELS_API_KEY_URL={vidgen_agent.PEXELS_API_KEY_URL}", text
            )
    ...
    def test_environment_keys_are_written_without_being_logged(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(
                os.environ,
                {
                    "VIDGEN_LLM_PROVIDER": "deepseek",
                    "VIDGEN_LLM_API_KEY": llm_key,
                    "VIDGEN_PEXELS_API_KEY": pexels_key,
                },
                clear=True,
            ), redirect_stdout(output):
                vidgen_agent.apply_environment_config(config_path)
    ...
    def test_material_key_check_matches_selected_source(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            _, default_missing = vidgen_agent.missing_config(config_path, [])
            _, pixabay_missing = vidgen_agent.missing_config(
    ...
    def test_existing_provider_key_is_reused_without_asking_user(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with redirect_stdout(output):
                provider = vidgen_agent.reuse_existing_llm_provider(config_path)
            _, missing = vidgen_agent.missing_config(config_path, [])
    ...
    def test_only_missing_pexels_key_does_not_ask_for_llm_again(self):
        ...
        with redirect_stdout(output):
            code = vidgen_agent.report_missing_config(
                "deepseek", ["pexels_api_keys"]
            )

        text = output.getvalue()
        self.assertEqual(code, vidgen_agent.NEEDS_INPUT_EXIT_CODE)
        self.assertIn(f"PEXELS_API_KEY_URL={vidgen_agent.PEXELS_API_KEY_URL}", text)
    ...
    def test_custom_openai_compatible_provider_requires_connection_details(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            provider, missing = vidgen_agent.missing_config(config_path, [])
            output = io.StringIO()
            with redirect_stdout(output):
                vidgen_agent.report_missing_config(provider, missing)
    ...
    def test_custom_openai_compatible_environment_is_mapped_to_oneapi(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(
                os.environ,
                {
                    "VIDGEN_LLM_PROVIDER": "openai_compatible",
                    "VIDGEN_LLM_API_KEY": "custom-key",
                    "VIDGEN_LLM_BASE_URL": "https://llm.example.com/v1",
                    "VIDGEN_LLM_MODEL_NAME": "example-model",
                },
                clear=True,
            ):
                vidgen_agent.apply_environment_config(config_path)
    ...
    def test_zip_extraction_rejects_parent_directory_escape(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(archive_path) as archive, self.assertRaises(
                vidgen_agent.SkillError
            ):
                vidgen_agent._safe_extract(archive, destination)
    ...
    def test_pexels_validation_filters_rejected_keys_without_logging_values(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def validate(request, timeout):
                self.assertEqual(
                    request.full_url, vidgen_agent.PEXELS_VALIDATION_URL
                )
                if request.get_header("Authorization") == bad_key:
                    raise vidgen_agent.urllib.error.HTTPError(
                        request.full_url, 401, "Unauthorized", None, None
                    )
                return self.FakeHttpResponse()

            with patch.object(
                vidgen_agent.urllib.request, "urlopen", side_effect=validate
            ), redirect_stdout(output):
                valid = vidgen_agent.validate_pexels_config(config_path, [])
    ...
    def test_pexels_validation_requests_new_key_when_all_are_rejected(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            error = vidgen_agent.urllib.error.HTTPError(
                vidgen_agent.PEXELS_VALIDATION_URL,
                403,
                "Forbidden",
                None,
                None,
            ...
            with patch.object(
                vidgen_agent.urllib.request, "urlopen", side_effect=error
            ):
                valid = vidgen_agent.validate_pexels_config(config_path, [])
    ...
    def test_generation_returns_only_non_empty_final_video(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.object(vidgen_agent.shutil, "which", return_value="uv"),
                patch.object(vidgen_agent, "run_checked"),
                patch.object(vidgen_agent.uuid, "uuid4", return_value=task_id),
                patch.object(
                    vidgen_agent.subprocess, "run", side_effect=finish_cli
                ) as run_mock,
            ):
                videos, task_dir, log_path, result_path = vidgen_agent.generate_video(
                    root,
                    "测试主题",
            ...
            self.assertEqual(
                command[voice_index + 1], vidgen_agent.DEFAULT_VOICE_NAME
            )
    ...
    def test_generation_failure_prints_original_model_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch.object(vidgen_agent, "run_checked"),
                patch.object(vidgen_agent.uuid, "uuid4", return_value=task_id),
                patch.object(vidgen_agent.subprocess, "run", side_effect=reject_model),
                redirect_stderr(stderr),
                self.assertRaises(vidgen_agent.SkillError),
            ):
                vidgen_agent.generate_video(root, "测试主题", [])

            self.assertIn(model_error, stderr.getvalue())
            result = json.loads(
                vidgen_agent.result_manifest_path(root).read_text(encoding="utf-8")
            )
    ...
    def test_successful_dependency_sync_does_not_print_package_list(self):
        ...
        with patch.object(
            vidgen_agent.subprocess, "run", return_value=result
        ), redirect_stdout(stdout), redirect_stderr(stderr):
            vidgen_agent.run_checked(["uv", "sync", "--frozen"], cwd=Path.cwd())
