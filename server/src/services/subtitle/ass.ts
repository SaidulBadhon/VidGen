...
import { hexToRgb, resolveBackgroundColor, resolveSubtitleFontPath } from "../video/textRender.ts";
...
export function buildAssDocument(cues: SubtitleCue[], options: AssRenderOptions): string {
  // Substituted here rather than in assRenderOptionsFromParams because this is
  // the first point that has both the styling and the text it has to draw.
  const styled: AssRenderOptions = {
    ...options,
    fontPath: resolveSubtitleFontPath(options.fontPath, cues.map((cue) => cue.text).join(" ")),
  };
  ...
  const lines: string[] = [
    ...
    `PlayResX: ${Math.round(styled.width)}`,
    `PlayResY: ${Math.round(styled.height)}`,
    "",
    "[V4+ Styles]",
    STYLE_FORMAT,
    buildAssStyleLine(styled),
    "",
    "[Events]",
    EVENT_FORMAT,
  ];
  ...
  for (const cue of cues) {
    const dialogue = buildAssDialogueLine(cue, styled);
